...
describe('mongodb/', () => {
  ...
  it('should return upload, delete, get, and list on success', async () => {
    mongo = await mongodb();
    expect(Object.keys(mongo)).toEqual(['upload', 'delete', 'get', 'list']);
  });

  it('should upload an image', async () => {
    const id = await mongo.upload({ stream: image(), opts: { filename: 'london.png' } });
    expect(_.isString(id)).toBeTruthy();
  });

  it('should get an image', async () => {
    const id = await mongo.upload({ stream: image(), opts: { filename: 'london2.png' } });
    const result = await mongo.get({ id });
